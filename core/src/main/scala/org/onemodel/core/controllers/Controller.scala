  ...
  def editAttributeOnSingleLine(attributeIn: Attribute): Boolean = {
    ...
    attributeIn match {
      ...
      case rtre: RelationToRemoteEntity =>
        //%%TEST THIS to see if as expected one simply gets an unsupported xyz exception when trying it on an RTRE:
      ...
      if (!userWantsOut) {
        ...
        if (newAttribute.isEmpty) {
          ...
          if (! (newAttribute.get.isInstanceOf[RelationToLocalEntity] || newAttribute.get.isInstanceOf[RelationToRemoteEntity])) {
            //%%IN DEBUG: what if is an RTRE here: does it get here? does it throw the error correctly fr next edit line?
